...
└── [메인 탭]
    ...
    │
    ├── 작업현황 탭 (신규)
    │   ├── 실행 중인 백그라운드 작업 목록 (스캔 · 백테스트 · 리포트 생성 · 캐시 업데이트)
    │   ├── 작업별 진행 메시지 표시
    │   └── 스캔 중단 버튼 (밸류돌파/눌림목/추세추종/하락방어/모멘텀 및 세력탐지)
    │
    └── 스캐너 탭 — 추세추종 모드 (scan_mode="trend")
        ├── 필터: RS90+ · 절대강도(EMA10>EMA20·200일선) · 둘 다
        ├── 진입 신호: EMA10/20/60 눌림목(-2~+3%) · 20/55/252일 신고가 돌파 · 박스권 돌파
        ├── EV 순위: (승률×손익비)-(1-승률), 반감기 3년 가중 백테스트 (60거래일 보유)
        └── 결과 테이블: 종목 · 진입신호 · RS점수 · EV · 승률 · 손익비 · 평균이익/손실 · 표본
```
