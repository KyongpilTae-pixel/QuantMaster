...
모드
  ...
  - LIVE   : APP_KEY/APP_SECRET 입력 시 KIS 실데이터로 전환(국내·미국 주식).

사용
  1) (선택) CONFIG에 KIS 키 입력 → LIVE 모드
  2) python quantletter_pipeline.py
  3) out_weekly.html / out_monthly.html 확인 → 발송 도구(Stibee 등)에 붙여넣기
픽 히스토리(picks_history.json)를 남겨, 다음 달 성적표를 자동 계산합니다.
"""
import os, json, time, datetime as dt
import pandas as pd, numpy as np

# ============================ CONFIG ============================
APP_KEY    = os.environ.get("KIS_APP_KEY","")      # 넣으면 LIVE
...
def momentum_rank(prices, lookback_days):
    ...
    last=p.index[-1]
    def ret(days):
        idx=p.index[p.index<=last]
        if len(idx)<=days: return pd.Series(dtype=float)
        return p.loc[last]/p.iloc[-1-days]-1
    m6=ret(126); m3=ret(63); m12=ret(252)
    vol=p.pct_change().iloc[-252:].std()*np.sqrt(252)
    tbl=pd.DataFrame({'m6':m6,'m3':m3,'m12':m12,'vol':vol}).dropna(subset=['m6'])
    tbl['score']=tbl['m6'].rank(pct=True)*0.6+tbl['m12'].rank(pct=True).fillna(0.5)*0.4
...
def sample_macro():
    def msi(s): s=s.copy(); s.index=s.index.to_period('M').to_timestamp(); return s[~s.index.duplicated(keep='last')]
    gold=msi(pd.read_csv('d_gold.csv',index_col=0,parse_dates=True)['gold'])
    spx=pd.read_csv('d_spx.csv',index_col=0,parse_dates=True); spx.index=spx.index.to_period('M').to_timestamp(); spx=spx[~spx.index.duplicated(keep='last')]
    ...
    y=bond/100.0; br=pd.Series(index=y.index,dtype=float); N=20; n=np.arange(1,N+1)
    for i in range(1,len(y)):
        y0,y1=y.iloc[i-1],y.iloc[i]; c=y0/2; cf=np.full(N,c); cf[-1]+=1
        br.iloc[i]=y0/12+((cf/(1+y1/2)**n).sum()-1)
    fxr=fx.pct_change()
    def krw(r): return (1+r)*(1+fxr)-1
    R=pd.DataFrame({'한국주식':kospi.pct_change(),'미국주식':krw(us_stk),
                    '미국채권':krw(br),'금':krw(gold.pct_change())}).dropna()
    return R

class KIS:
    def __init__(self):
        import requests; self.r=requests
        self.base="https://openapivts.koreainvestment.com:29443" if IS_PAPER else "https://openapi.koreainvestment.com:9443"
        self.tok=None
    def token(self):
        if self.tok: return self.tok
        r=self.r.post(f"{self.base}/oauth2/tokenP",headers={"content-type":"application/json"},
            data=json.dumps({"grant_type":"client_credentials","appkey":APP_KEY,"appsecret":APP_SECRET}))
        r.raise_for_status(); self.tok=r.json()["access_token"]; return self.tok
    def _h(self,tr): return {"content-type":"application/json","authorization":f"Bearer {self.token()}",
                             "appkey":APP_KEY,"appsecret":APP_SECRET,"tr_id":tr,"custtype":"P"}
    def daily_kr(self, code, start, end):
        u=f"{self.base}/uapi/domestic-stock/v1/quotations/inquire-daily-itemchartprice"
        p={"FID_COND_MRKT_DIV_CODE":"J","FID_INPUT_ISCD":code,"FID_INPUT_DATE_1":start,
...
def live_stock_prices(universe, market):
    k=KIS(); end=dt.date.today().strftime("%Y%m%d")
...
US_UNIVERSE=["AAPL","MSFT","NVDA","AMZN","META","GOOGL","AVGO","TSLA","JPM","V",
             "LLY","UNH","XOM","MA","COST","HD","NFLX","AMD","CRM","ADBE"]
KR_UNIVERSE=["005930","000660","373220","207940","005380","000270","068270","035420","012450","042660"]
...
def save_pick(hist, asof, picks):
    hist["weekly"].append({"asof":str(asof.date() if hasattr(asof,'date') else asof),
                           "tickers":[p["ticker"] for p in picks]})
    json.dump(hist, open(HISTORY,"w"), ensure_ascii=False, indent=1)

def scorecard(prices, hist, weeks_ago=4):
    if len(hist["weekly"])<1: return None
    ...
    return {"from":prev["asof"],"to":str(p.index[-1].date()),"rows":rows,
            "avg":round(float(fwd.mean()),2),"mkt":round(float(mkt),2),
            "beat":int((fwd>mkt).sum()),"up":int((fwd>0).sum()),"n":len(tks)}

def render(weekly_tbl, asof, alloc_w, alloc_s, sc):
    from newsletter_render import weekly_html, monthly_html
    open('out_weekly.html','w').write(weekly_html(BRAND, asof, weekly_tbl, TOP_N, MARKET, LOOKBACK_M))
    open('out_monthly.html','w').write(monthly_html(BRAND, alloc_w, alloc_s, sc))
...
def main():
    ...
    picks=[{"ticker":t,"m6":round(r.m6*100,1),"m3":round(r.m3*100,1),
            "m12":round(r.m12*100,1),"vol":round(r.vol*100,1)}
           for t,r in tbl.head(TOP_N).iterrows()]
    w,s=alloc_weights(macro)
    hist=load_history()
    sc=scorecard(prices, hist)
    save_pick(hist, asof, picks)
    render({"picks":picks,"asof":str(asof.date() if hasattr(asof,'date') else asof)}, asof, w, s, sc)
    ...
    if sc: print("성적표:", f"추천 {sc['avg']}% vs 시장 {sc['mkt']}% (초과 {sc['beat']}/{sc['n']})")
    else:  print("성적표: 히스토리 부족(다음 실행부터 생성)")
